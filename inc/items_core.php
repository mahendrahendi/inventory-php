...
class Items {
	...
	public function update_item($itemid, $name, $desc, $cat, $price, $imageup) {
		// Create log
		$update = $this->new_log(3, $itemid, false, $price);

		$prepared = $this->prepare("UPDATE invento_items SET name=?, descrp=?, category=?, price=?, img=? WHERE id=?", 'update_item()');
		$this->bind_param($prepared->bind_param('sssssi', $name, $desc, $cat, $price, $imageup, $itemid), 'update_item()');
	...
	public function update_item2($itemid, $name, $desc, $cat, $price, $imageup) {
		// Create log
		$update = $this->new_log(3, $itemid, false, $price);
